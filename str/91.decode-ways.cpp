...
private:
  void backtrace(std::string s, int len, int start, int end) {
    // 1. 如果substr大于26或substr=0，返回start+1,end=start+1

    // 2. 如果end == len-1, substr在[1,26]之间，ans++,return

    // 3. 从end开始到len，如果子串在[1,26],ans++, i++;否则 start+1开始递归
  }
